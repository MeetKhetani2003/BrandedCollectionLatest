              ...
              <div key={order._id} className="border-t">
                ...
                <div className="grid grid-cols-[1.2fr_1.5fr_1fr_2fr_1fr_0.7fr_0.7fr] px-6 py-4 text-sm items-start hover:bg-gray-50 transition">
                  ...
                  <div className="space-y-2">
                    {/* ORDER STATUS */}
                    {!order.awbNumber ? (
                      <span className="inline-block text-xs font-medium text-purple-700 bg-purple-50 border border-purple-100 px-2 py-1 rounded-full">
                        Order Placed
                      </span>
                    ...
                    <div className="flex gap-1 mt-1">
                      <input
                        ...
                        onKeyDown={async (e) => {
                          ...
                          try {
                            // 1️⃣ Save AWB first
                            const saveRes = await fetch("/api/order/awb", {
                              method: "PATCH",
                            ...
                            if (!saveRes.ok || !saveData.success) {
                              alert(
                                "Failed to save AWB: " +
                                  (saveData.message || "Unknown error"),
                              );
                              setTrackingLoading(null);
                              return;
                            }

                            // 2️⃣ Then fetch tracking
                            ...
                            if (!trackRes.ok || !trackData.success) {
                              alert(
                                "Failed to fetch tracking: " +
                                  (trackData.message || "Unknown error"),
                              );
                              setTrackingLoading(null);
                              return;
                            }
                      ...
                      <button
                        ...
                        onClick={async (e) => {
                          ...
                          const inputAwb = input ? input.value.trim() : "";

                          let targetAwb = order.awbNumber;

                          // If input has a value, try to save it first
                          if (inputAwb) {
                            if (inputAwb !== order.awbNumber) {
                              setTrackingLoading(order._id);
                              try {
                                const saveRes = await fetch("/api/order/awb", {
                                  method: "PATCH",
                                ...
                                alert("Error saving AWB: " + err.message);
                                setTrackingLoading(null);
                                return;
                              }
                            }
                          ...
                          if (!targetAwb) {
                            alert("No AWB number to fetch tracking for.");
                            return;
                          }
                          ...
                          try {
                            ...
                            alert("Error fetching tracking: " + error.message);
                          } finally {
                            setTrackingLoading(null);
                          }
                        }}
